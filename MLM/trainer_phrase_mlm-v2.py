...
class JoImTeR(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step in pbar:
                ...
                words_features, sent_code, _, _, _, _ = image_encoder(
                ...
                w_loss0, w_loss1, _ = words_loss(words_features, words_embs[:, :, 1:], labels, cap_lens - 1, class_ids,
                                                 batch_size)
                w_total_loss0 += w_loss0.item()
                w_total_loss1 += w_loss1.item()
                damsm_loss += w_loss0 + w_loss1

                bg_loss0, bg_loss1, _ = words_loss(words_features, bigram_embs[:, :, 1:], labels,
                ...
                tg_loss0, tg_loss1, _ = words_loss(words_features, trigram_embs[:, :, 1:], labels,
                ...
                w_t_loss0, w_t_loss1, _ = words_triplet_loss(words_features, words_embs[:, :, 1:], labels, neg_ids,
                ...
                # bigrams
                bg_t_loss0, bg_t_loss1, _ = words_triplet_loss(words_features, bigram_embs[:, :, 1:], labels, neg_ids,
                ...
                # trigrams
                tg_t_loss0, tg_t_loss1, _ = words_triplet_loss(words_features, trigram_embs[:, :, 1:], labels, neg_ids,
    ...
    def evaluate(self, cnn_model, trx_model):
        ...
        for step in tqdm(range(len(val_data_iter)), leave=False):  #
            ...
            bg_loss0, bg_loss1, attn = words_loss(words_features, bigram_embs[:, :, 1:], labels,
                                                  cap_lens - 1, class_ids, batch_size)
            bg_total_loss += (bg_loss0 + bg_loss1).item()
            tg_loss0, tg_loss1, attn = words_loss(words_features, trigram_embs[:, :, 1:], labels,
                                                  cap_lens - 1, class_ids, batch_size)
