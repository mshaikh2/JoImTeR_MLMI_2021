...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        trigram_feat = self.trigram_fc(trigram)  # [B, L, C]
        word_feat = self.word_fc(word_feat)  # [B, L, C]
        return word_feat.transpose(1, 2), bigram_feat.transpose(1, 2), trigram_feat.transpose(1, 2), sent_feat
...
class TextEncoderMLM(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        pred = self.mlm(
            self.LayerNorm(self.act(self.transform(word_feat))))  # [B, timestep, 512] --> [B, timestep, 8410]
...
#         ## text feature input ##
#         self.self.proj_sent = nn.Linear(cfg.hidden_dim, 256) # projection global sentence feature, 512 --> 256
