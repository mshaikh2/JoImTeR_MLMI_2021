...
class SoftAttention(nn.Module):
    def __init__(self, in_groups, m_heads, in_channels):
        ...
        self.conv3d = nn.Conv3d(in_channels=in_groups, out_channels=m_heads, kernel_size=(in_channels, 1, 1),
...
class ImageEncoder_Classification(nn.Module):
    ...
    def forward(self, x):
        f_r, f_g, attn_maps_l4, learnable_scalar_l4, attn_maps_l5, learnable_scalar_l5 = self.pretrained_encoder(
            x)  # N, 512, 16, 16; N, 512
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        bigram = self.norm(self.GELU(self.bigram(word_feat.transpose(1, 2))).transpose(1,
                                                                                       2))  # conv1d expects [B, C, L], --> [B, L-1, C]
        #         print('bigram.shape:',bigram.shape)
        trigram = self.norm(self.GELU(self.trigram(word_feat.transpose(1, 2))).transpose(1,
                                                                                         2))  # conv1d expects [B, C, L], --> [B, L-2, C]
...
class ImageText_Classification(nn.Module):
    def __init__(self, num_class=14, img_encoder_path=None, txt_encoder_path=None, pretrained=False, cfg=None,
                 bert_config=None):
    ...
    def forward(self, x, t, mask):
        ## image input ##
        f_r, f_g, attn_maps_l4, learnable_scalar_l4, attn_maps_l5, learnable_scalar_l5 = self.img_encoder(
            x)  # N, 512, 16, 16; N, 512
        #         f_r,attn_maps = self.soft_attention(f_r)
        g_p = torch.squeeze(self.gap(f_r))  # N, 512
        ...
        f_w_avg = (f_w * mask.unsqueeze(-1)).sum(1) / mask.sum(1).unsqueeze(
            -1)  # [N, 512] masked average pooling from word_feat
        f_b_avg = (f_b * mask[:, :-1].unsqueeze(-1)).sum(1) / mask[:, :-1].sum(1).unsqueeze(
            -1)  # [N, 512] masked average pooling from bigram_feat
        f_t_avg = (f_t * mask[:, :-2].unsqueeze(-1)).sum(1) / mask[:, :-2].sum(1).unsqueeze(
            -1)  # [N, 512] masked average pooling from trigram_feat
