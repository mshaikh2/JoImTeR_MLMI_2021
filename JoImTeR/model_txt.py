...
class ImageText_Classification(nn.Module):
    ...
    def forward(self, x, t, mask):
        ...
        f_w_avg = (f_w * mask.unsqueeze(-1)).sum(1) / mask.sum(1).unsqueeze(-1)
        # [N, 512] masked average pooling from word_feat
        f_b_avg = (f_b * mask[:, :-1].unsqueeze(-1)).sum(1) / mask[:, :-1].sum(1).unsqueeze(-1)
        # [N, 512] masked average pooling from bigram_feat
        f_t_avg = (f_t * mask[:, :-2].unsqueeze(-1)).sum(1) / mask[:, :-2].sum(1).unsqueeze(-1)
        # [N, 512] masked average pooling from trigram_feat
