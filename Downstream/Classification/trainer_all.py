...
#                                         (s_total_loss0 + s_total_loss1) / (step+1) ) )
                pbar.set_description('lr: %.3e %.3e, w_l: %.4f, s_l: %.4f, w_t: %.4f, s_t: %.4f' 
                                     % ( optimizerI.param_groups[0]['lr'], optimizerT.param_groups[0]['lr'],
                                        (w_total_loss0 + w_total_loss1) / (step+1),
                                        (s_total_loss0 + s_total_loss1) / (step+1),
                ...
                ######################################################################################################
                ##########################################################
            v_s_cur_loss, v_w_cur_loss, v_s_t_cur_loss, v_w_t_cur_loss = self.evaluate(image_encoder, text_encoder)
