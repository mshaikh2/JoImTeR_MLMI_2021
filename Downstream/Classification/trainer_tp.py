...
#         lambda1 = lambda step: 0.3 ** (step / batch_step) # stepwise
        ...
        ###### manually set lr in optimizers here #######
        ###### lr_scheduler is also committed in train() ######
        optimizerI.param_groups[0]['lr'] = 1e-4
        optimizerT.param_groups[0]['lr'] = 1e-4
...
#                                         (s_total_loss0 + s_total_loss1) / (step+1) ) )
                pbar.set_description('lr: %.4e %.4e, triplet %.4f, w_tloss: %.4f, s_tloss: %.4f' 
                                     % ( optimizerI.param_groups[0]['lr'], optimizerT.param_groups[0]['lr'],
                                         total_t_loss / (step+1), 
                ...
                ######################################################################################################
                ##########################################################
            v_s_t_cur_loss, v_w_t_cur_loss = self.evaluate(image_encoder, text_encoder)
            print('[epoch: %d] val_w_t: %.4f, val_s_t: %.4f' % (epoch, v_w_t_cur_loss, v_s_t_cur_loss))
            ### val losses ###
...
#         w_cur_loss = w_total_loss / (step+1)
        ...
        return s_t_cur_loss, w_t_cur_loss # s_cur_loss, w_cur_loss
