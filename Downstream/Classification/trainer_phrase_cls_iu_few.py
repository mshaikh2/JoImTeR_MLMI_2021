...
from model_phrase_cls import ImageText_Classification
...
#             self.class_weight_vector = self.class_weight_vector.cuda()
    ...
    def build_models(self):
        # ################### encoders ################################# #      
        main_model = ImageText_Classification(num_class=self.data_loader.dataset.num_classes, 
                                                 img_encoder_path=cfg.init_image_encoder_path, 
...
#         print(main_model)
        ...
        for n, p in main_model.named_parameters():
            if ('img_encoder' not in n) and ('txt_encoder' not in n):
#                 print(n)
        ...
        if cfg.freeze_backbone:
            ...
            for p in main_model.img_encoder.parameters(): 
                p.requires_grad = False
            main_model.txt_encoder.eval() ## turn off BN and Dropout
            for p in main_model.txt_encoder.parameters(): 
                p.requires_grad = False
        else:
            optim_params.append({'params':list(main_model.img_encoder.parameters()) + list(main_model.txt_encoder.parameters()), 'lr':cfg.lr_backbone})
...
#             lr_schedulerT = torch.optim.lr_scheduler.StepLR(optimizerT, cfg.lr_drop, gamma=cfg.lr_gamma)
        ...
        if cfg.text_encoder_path != '':
            model_path = cfg.text_encoder_path #.replace('text_encoder', 'image_encoder')
...
#                     lr_schedulerI.load_state_dict(state_dict['lr_scheduler'])
    ...
    def save_model(self, main_model, optimizerI, lr_schedulerI, epoch):
        ...
        }, '%s/ITclass_model%d.pth' % (self.model_dir, epoch))
...
#                 break
                ...
                ######################################################
                imgs, captions, cap_masks, classes, uids, cap_lens = data_iter.next()
                ...
                if cfg.CUDA:
                    imgs, captions, cap_masks, classes = imgs.cuda(), captions.cuda(), cap_masks.cuda(), classes.cuda()
                
                ################## feedforward damsm model ##################
                main_model.zero_grad() # image encoder zero_grad here
#                 optimizerI.zero_grad()
                
                y_pred = main_model(imgs, captions, cap_masks) # feedforward
...
#                 tbw.add_scalar('Val_step/{0}'.format(self.data_loader.dataset.idx_to_class[idx]), auc_scores[idx], epoch)
            ...
            for idx in range(len(auc_scores)):
                print('%s: %.4f' % (self.data_loader.dataset.idx_to_class[openi_cls[idx]], auc_scores[idx]))

            avg = np.mean(np.array(auc_scores)[[0,1,2,3,5,6,7]])
            print('Avg: %.4f' % avg)

            # weight = np.array([295, 319, 28, 41, 2988, 141, 36, 25]) # weight for the whole open-i dataset
            few = 5
            weight = np.array([295, 319, 28, 41, 141, 36, 25]) - few # weight for the whole open-i dataset
            weight = weight / weight.sum()
            wavg = np.array(auc_scores)[[0,1,2,3,5,6,7]] @ weight
            print('wAvg: %.4f' % wavg)
...
#         self.save_model(image_encoder, text_encoder, optimizerI, optimizerT, lr_schedulerI, lr_schedulerT, epoch)                
    ...
    def evaluate(self, cnn_model):
        ...
        for step in tqdm(range(len(val_data_iter)), leave=False):  
            real_imgs, captions, cap_masks, classes, uids, cap_lens = val_data_iter.next()
            if cfg.CUDA:
                real_imgs, captions, cap_masks, classes = real_imgs.cuda(), captions.cuda(), cap_masks.cuda(), classes.cuda()
            
            y_pred = cnn_model(real_imgs, captions, cap_masks)
